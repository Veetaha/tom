...
pub fn generate_syntax(mode: Mode) -> Result<()> {
    let syntax_kinds_file = project_root_dir().join(codegen::SYNTAX_KINDS);
    let syntax_kinds = generate_syntax_kinds(KINDS_SRC)?;
    verify_or_overwrite(syntax_kinds_file.as_path(), &syntax_kinds, mode)?;

    let ast_tokens_file = project_root_dir().join(codegen::AST_TOKENS);
    let contents = generate_tokens(AST_SRC)?;
    verify_or_overwrite(ast_tokens_file.as_path(), &contents, mode)?;

    let ast_nodes_file = project_root_dir().join(codegen::AST_NODES);
    let contents = generate_nodes(KINDS_SRC, AST_SRC)?;
    verify_or_overwrite(ast_nodes_file.as_path(), &contents, mode)?;
